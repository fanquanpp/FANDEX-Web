...
> **FANDEX** — 开发者知识库 | Astro 5 + Vue 3 | 51 模块 · 1993 篇文档

---

## 1. 项目概述

FANDEX 是一个基于 Astro 5 SSG 的开发者知识库站点，使用 Vue 3 岛屿架构实现客户端交互。项目覆盖 51 个技术模块、1993 篇文档、27 篇术语表，部署在 GitHub Pages 上。
...
- 托管层：GitHub Pages 静态托管
- 构建层：Astro 5 SSG 构建（Content Markdown/MDX → Astro Pages & Components → Vue 3 Islands）
- 构建流水线：build-glossary-index.mjs → build-search-index.mjs → astro build → pagefind
- 客户端增强：Service Worker、术语提示、代码运行器、Mermaid 渲染、KaTeX 数学、Admonition 样式
...
| **CheatSheet**     | `CheatSheet.vue`     | `client:load`    | 语法速查组件，支持搜索过滤、代码复制、分组折叠、学习指引                       |
...
| **modules**            | `modules.ts`            | 51 个模块定义、categories 数组、categoryColors 颜色体系、icon 字段、前置关系图、查询函数 |
...
51 个模块目录，每个包含多篇 Markdown 文档，frontmatter 遵循 Zod Schema：
...
| `eng-infra` | 工程与基础设施 | `#e05a2b` | 运维 · 网络技术 · 网络安全 · 云计算 · 物联网 · 软件测试 · AI Agent · AI 工程 · 软件工程 · 软件架构 · 工程实践                      |
| `data`      | 数据技术       | `#f9a825` | 数据分析 · 大数据 · 机器学习 · 深度学习 · AI 工程 · 计算机视觉 · 自然语言处理 · 大语言模型 · 生成式 AI · 多模态 AI · AI 伦理与安全 |
