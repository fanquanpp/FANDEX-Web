...
**解析**: SQL 中 `NULL` 与任何值的比较（包括 `NULL = NULL`）都返回 `NULL`，而非 `TRUE` 或 `FALSE`。判断是否为 `NULL` 必须使用 `IS NULL` 或 `IS NOT NULL`。这是 SQL 三值逻辑的核心要点。
</details>
...
-- 检查是否更新成功（受影响行数为 0 表示余额不足）
-- 应用层判断 ROW_COUNT()，若为 0 则 ROLLBACK
