...
> 跨模块引用：[[os|操作系统]]的内核架构（宏内核vs微内核）体现了开闭原则和接口隔离原则。[[network|计算机网络]]的协议栈分层是单一职责和依赖倒置的体现。[[architecture|体系结构]]的ISA是依赖倒置的经典案例。
...
注意:
  ...
  [[java/overview|Java]]实现: 枚举单例 (防反射和序列化)
  [[cpp/overview|C++]]实现: Meyers' Singleton (局部静态变量)
```
...
浅拷贝 vs 深拷贝:
  ...
  [[java/overview|Java]]: Object.clone() 默认浅拷贝
  [[cpp/overview|C++]]: 拷贝构造函数, 需自行实现深拷贝
...
跨模块引用: [[os|操作系统]]的系统调用适配层是Adapter模式。
  [[network|计算机网络]]的协议转换器是Adapter模式。
```
...
与继承的区别:
  ...
  [[java/overview|Java]]: InputStream/OutputStream族是装饰器模式
  [[cpp/overview|C++]]: std::streambuf的派生类链
```
...
跨模块引用: [[os|操作系统]]的文件系统目录树是Composite模式。
  [[discrete-math|离散数学]]的树结构是Composite的数学基础。
```
...
跨模块引用: [[os|操作系统]]的系统调用是内核子系统的Facade。
  [[compiler|编译原理]]的编译器前端是后端子系统的Facade。
```
...
跨模块引用: [[java/overview|Java]]的动态代理 (java.lang.reflect.Proxy)。
  [[cpp/overview|C++]]的智能指针 (shared_ptr) 是智能引用代理。
  [[network|计算机网络]]的反向代理 (Nginx) 是远程代理。
```
...
跨模块引用: [[os|操作系统]]的信号机制是Observer模式。
  [[network|计算机网络]]的事件驱动IO (epoll) 是Observer模式。
  [[java/overview|Java]]的Observable/Observer (已废弃) 和PropertyChangeListener。
```
...
应用:
  ...
  [[os|操作系统]]的Shell命令是Command模式。
  [[java/overview|Java]]的Runnable是Command模式。
```
...
跨模块引用: [[os|操作系统]]的管道(pipe)和消息队列是生产者-消费者模式。
  [[network|计算机网络]]的TCP缓冲区是生产者-消费者模式。
```
...
跨模块引用: [[os|操作系统]]的读写锁 (pthread_rwlock)。
  [[java/overview|Java]]的ReentrantReadWriteLock。
  [[architecture|体系结构]]的缓存一致性协议 (MESI) 是读写锁的硬件实现。
```
...
跨模块引用: [[java/overview|Java]]的ExecutorService是线程池。
  [[os|操作系统]]的进程池和线程池。
  [[cpp/overview|C++]]的std::async底层使用线程池。
```
